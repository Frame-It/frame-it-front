...
const UserRegisterCheckbox = React.forwardRef<
  React.ElementRef<typeof CheckboxPrimitive.Root>,
  React.ComponentPropsWithoutRef<typeof CheckboxPrimitive.Root> & {
    isNotBackground?: boolean;
  }
>(({ className, isNotBackground, ...props }, ref) => (
  <CheckboxPrimitive.Root
    ref={ref}
    className={cn(
      'h-[18px] w-[18px] shrink-0 rounded-full ring-offset-background',
      'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
      'disabled:cursor-not-allowed disabled:opacity-50',
      isNotBackground
        ? 'border-none text-gray-70 data-[state=checked]:bg-transparent data-[state=checked]:text-primary'
        : 'border border-primary data-[state=checked]:bg-primary data-[state=checked]:text-white',
      className,
    )}
    {...props}
  >
    {isNotBackground ? (
      <div className="flex items-center justify-center">
        <Icon id="check-icon" className="size-[16px]" />
      </div>
    ) : (
      <CheckboxPrimitive.Indicator
        className={cn('flex items-center justify-center text-current')}
      >
        <Icon id="check-icon" className="h-[14px] w-[14px]" />
      </CheckboxPrimitive.Indicator>
    )}
  </CheckboxPrimitive.Root>
));
